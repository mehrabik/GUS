...
// CAGADroid: Start
import android.cagadroid.sm.CAGADroidSecurityManager;
...
public class PermissionManagerService {
    ...
    /** CAGADroid SM Instance (Start) **/
    private static CAGADroidSecurityManager mCAGADroidService;
    /** CAGADroid SM Instance (End) **/
    ...
    // CAGADroid: function to check the permission with SM
    private int checkCAGADroidPermission(String permName, PackageParser.Package pkg, int uid,
            int callingUid) {
        // When we are here, the permission would have been granted to the app by the system if there were no CAGADroid
        // So, we just want to double check and see if CAGADroid would deny it
        // Therefore the default policy is to allow the permission
        Log.w("CAGA.PE", "Performing CAGADroid permission check: PermissionName=" + permName
                                    + ", UID=" + uid
                                    + ", CallingUID=" + callingUid
                                    + ", PackageName=" + pkg.packageName
                                );

        if (mCAGADroidService == null) {
            Log.w("CAGADroid", "CAGADroidSecurityManager: Reinstantiating CAGADroid PolicyCheck Service");
            mCAGADroidService = CAGADroidSecurityManager.getInstance();
        }
        
        if(mCAGADroidService.checkPolicy(pkg.packageName, permName,
            CAGADroidConstants.OPERATION_PERMISSION_REQUEST) == CAGADroidConstants.ACTION_DENY) {
            return PackageManager.PERMISSION_DENIED;
        }
        return PackageManager.PERMISSION_GRANTED;
    }
    ...
    private int checkUidPermission(String permName, PackageParser.Package pkg, int uid,
        ...
        if (pkg != null) {
            ...
            if (permissionsState.hasPermission(permName, userId)) {
                if (isUidInstantApp) {
                    if (mSettings.isPermissionInstant(permName)) {
                        //CAGADroid: This is where PermissionLocker check should be performed
			 return checkCAGADroidPermission(permName, pkg, uid, callingUid);
                    }
                } else {
		            //CAGADroid: This is where PermissionLocker check should be performed
		            return checkCAGADroidPermission(permName, pkg, uid, callingUid);
            ...
            if (isImpliedPermissionGranted(permissionsState, permName, userId)) {
                //CAGADroid: Implied permissions are not relevant to PermissionLocker, so perform the check anyway
                return checkCAGADroidPermission(permName, pkg, uid, callingUid);
            }
        } else {
            ...
            if (perms != null) {
                if (perms.contains(permName)) {
                    //CAGADroid: Ignore system permissions as it may interfere with stable operation of the phone
                    return PackageManager.PERMISSION_GRANTED;
                }
