...
class ActivityStarter {
    ...
    // CAGADroid: function to check start Activity with CAGADroid
    private int checkCAGADroidAccess(String packageName, String className) {
        return CAGADroidSecurityManager.getInstance().checkPolicy(packageName, className,
                                CAGADroidConstants.OPERATION_START_ACTIVITY);
    }
    // CAGADroid: end
    ...
    private int startActivity(final ActivityRecord r, ActivityRecord sourceRecord,
        ...
        // CAGADroid: start ActivityLocker check
        String subject = "Unknwon";
        String object = "Unknown";
        if(r.intent.getComponent() != null) {
            subject = r.intent.getComponent().getPackageName();
            object = r.intent.getComponent().getClassName();
        }
        Log.d("CAGA.PE", "ActivitStarter.startActivity: Subject=" + subject
                                                      + ", Object=" + object);
        if(checkCAGADroidAccess(subject, object) == CAGADroidConstants.ACTION_DENY) {
        ...
        try {
            ...
            if (ActivityManager.isStartResultSuccessful(result)) {
                if (startedActivityStack != null) {
                    ...
                    // CAGADroid: start
                    Boolean CAGACheck = true;
                    if (currentTop != null && currentTop.intent.getComponent() != null) {
                        subject = currentTop.intent.getComponent().getPackageName();
                        object = currentTop.intent.getComponent().getClassName();
                        Log.d("CAGA.PE", "ActivitStarter.startActivity: Subject=" + subject
                                                      + ", Object=" + object);
                        if(checkCAGADroidAccess(subject, object) == CAGADroidConstants.ACTION_DENY)
                        {
                            Log.v("CAGA.PE", "Top activity is not allowed. Closing");
                            CAGACheck = false;
                            final ActivityStack rStack = currentTop.getActivityStack();
                            if (rStack != null) {
                                rStack.finishActivityLocked(currentTop, RESULT_CANCELED,
                                        null, "topActivity", true);
                            } else{
                                Log.w("CAGA.PE", "Top activity stack is null");
                            }
                    ...
                    if (CAGACheck && currentTop != null && currentTop.shouldUpdateConfigForDisplayChanged()) {
                    // CAGADroid: end
